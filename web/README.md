...
| 承認フロー定義 | **admin のみ**。上段が**フロー定義の一覧**（プロセスごとに1行）、下段が選んだ定義の**配備履歴**。BPMN の配備、切り戻し、停止・再開。配備した時点から新規の起票が新しい版で始まる（アプリの再起動は不要） |
...
`GET /api/admin/process-definitions` は全プロセスの版を平らに返すため、プロセスが増えると版が
混ざって読めない。画面側で `groupProcessDefinitions()`（`src/lib/process-definitions.ts`）を使い、
プロセスごとにまとめ直してから2段に分けて表示している。
