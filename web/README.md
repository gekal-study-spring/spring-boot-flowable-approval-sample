...
| 承認フロー図 | BPMN 図を bpmn-visualization で描画し、通過済み（緑）・実行中（橙）を塗り分ける。拡大・縮小・全体表示のボタン、ドラッグで移動、「全画面」ボタンで全画面表示（Esc または閉じるボタンで戻る） |
...
| --- | --- | --- |
| 移動 | ドラッグ | ドラッグ |
| 拡大縮小 | ボタン、`Ctrl + ホイール`、キーボードの `+` `-` | 左に加えてホイール |
| 全体表示に戻す | ボタン、キーボードの `0` | 同左 |

ホイールの扱いを表示状態で変えている。**通常表示では素のホイールをページのスクロールに使う**
（図はページの途中に埋まっているため、奪うと下へ進めなくなる）。全画面ではスクロールする先が
無いので、素のホイールを拡大縮小に読み替えている。

なお bpmn-visualization 自身が拡大縮小と見なすのは `Ctrl + ホイール` だけで、**⌘ では効かない**
（`event.ctrlKey && !event.metaKey` を見ている）。macOS のトラックパッドのピンチはブラウザが
`Ctrl + ホイール` として送るため、そのまま拡大縮小できる。
