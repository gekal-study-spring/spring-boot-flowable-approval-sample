...
**2本指スワイプは、図を動かしきったところでページのスクロールに引き継ぐ。** 入れ子のスクロール領域が
端で親に渡す macOS の挙動に揃えてあるので、図がページの途中にあっても下へ読み進められる。
図が入れ物に収まっているとき（＝全体表示のとき）は動かす余地がないので、最初からページが動く。
...
ピンチはブラウザが `Ctrl + ホイール` として送るため、ライブラリの拡大縮小がそのまま働く。
ただし判定は `event.ctrlKey && !event.metaKey` なので、**⌘ を押しても拡大縮小にはならない**。
